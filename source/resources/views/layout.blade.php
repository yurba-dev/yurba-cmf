    ...
    <div class="y-shell">
            ...
            @if($yurbaUser)
                <div class="y-sidebar__user">
                    ...
                    <form method="POST" action="{{ route('yurba.logout') }}">
                        @csrf
                        <button type="submit" class="y-btn y-btn__ghost y-btn__block">{{ __('Log out') }}</button>
                    </form>
                </div>
            @endif
        </aside>
        ...
        <div class="y-main">
            <header class="y-topbar">
                <button type="button" class="y-menu-toggle" aria-label="{{ __('Menu') }}" onclick="document.body.classList.toggle('y-nav-open')">≡</button>
                <div class="y-topbar__title">@yield('heading', '')@hasSection('titleCount')<span class="y-topbar__count" title="{{ __('Total records') }}">@yield('titleCount')</span>@endif</div>
                <div class="y-topbar__user">
                    @if($yurbaUser)
                        <span class="y-user">{{ $yurbaUser->name ?? $yurbaUser->email }}</span>
                        <form method="POST" action="{{ route('yurba.logout') }}">
                            @csrf
                            <button type="submit" class="y-btn y-btn__ghost">{{ __('Log out') }}</button>
                        </form>
                    @endif
                </div>
            </header>
            ...
            <footer class="y-footer">
                <span>Powered by <a href="https://dev.yurba.one" target="_blank" rel="noopener noreferrer">YurbaCMF</a></span>
                <span class="y-footer__version">v{{ \Yurba\Cmf\Panel::VERSION }}</span>
            </footer>
        </div>
    </div>
